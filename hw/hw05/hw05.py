...
def weight(size):
    ...
    assert size > 0, "weight size must be positive"
    return tree(size, [])
...
def is_weight(w):
    ...
    return is_tree(w) and not is_mobile(w) and not branches(w)
...
def balanced(m):
    ...
    if is_weight(m):
        return True
    left_m = end(sides(m)[0])
    left_torque = length(sides(m)[0]) * total_weight(left_m)
    right_m = end(sides(m)[1])
    right_torque = length(sides(m)[1]) * total_weight(right_m)
    return left_torque == right_torque and balanced(left_m) and balanced(right_m)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert not (lower_bound(y) <= 0 <= upper_bound(y)), 'Divisor maybe zero'
...
def quadratic(x, a, b, c):
    ...
    l, r, e = map(f, (lower_bound(x), upper_bound(x), extremum))
